...
class Node:
	...
	def __init__(self, nodeid: int, simulator):
		...
		self.send_pkt(self.simulator.cost[self.nodeid])
	...
	def update(self, pkt: Packet):
		...
		self_vector = self.dist_table[pkt.dest]
		old_vector = self_vector[:]
		for i in range(len(self_vector)):
			if self_vector[pkt.src] + pkt.dist_vector[i] < self_vector[i]:
				self_vector[i] = self_vector[pkt.src] + pkt.dist_vector[i]
				self.predecessors[i] = pkt.src

		# if some vector value in self_vector become small, tell neighbours
