...
struct A_side {
    int base;
    int nextseqnum;
    struct pkt sender_packets[100000]; 
};
...
struct A_side a_side;
struct B_side b_side;
/* 
...
/*
 * Make the packet, set attribute, and put it
 * into buffer
 */
struct pkt make_pkt(char* data, int seqnum, int acknum){
    struct pkt p;
    for (int i = 0; i < MSG_SIZE; i++){
        p.payload[i] = data[i];
    }
...
/* 
 * check if B receive the expected sequence number
 */
...
void A_send(struct msg message) {
    if (a_side.nextseqnum >= a_side.base + 8){  // if A receive data too quickly from app layer,
                                                // which greater than the window size 8, simply ignore
        printf("A refuse data from application layer\n");
        return;
    }
   struct pkt p = make_pkt(message.data, a_side.nextseqnum, 0);
    a_side.sender_packets[a_side.nextseqnum] = p;
    to_network_layer(0, p);
...
void A_recv(struct pkt packet) { 
    if (is_corrupted(packet) == 1) {     // if A receive corrupt data
        printf("A_recv: packet corrupted.\n");
        return;
    }
    if (a_side.base > packet.acknum){   // if A receive acknowledgement that greater than the base
        printf("A_recv: valid acknowledgement, but sequence number greater than the base.\n");
        return;
    }
    printf("A_recv: valid acknowledgement packet.\n");
    a_side.base = packet.acknum + 1;   // receive valid acknowledgement, set base
    stoptimer(0);
    if (a_side.base != a_side.nextseqnum){
        starttimer(0, 20);
    }
...
void A_timeout(void) {
    printf("timeout, resend packets between sequence number %d and sequence number %d\n", a_side.base, a_side.nextseqnum);
    starttimer(0, 20);
    for (int i = a_side.base; i < a_side.nextseqnum; i++){ // timeout resend all the messages that don't
        to_network_layer(0, a_side.sender_packets[i]);     // receive acknowledgements in the window
    }
}
...
void B_recv(struct pkt packet) {
    struct pkt p;
    char emptymessage[MSG_SIZE];
    char to_application_msg[MSG_SIZE];
    int has_seqnum = hasseqnum(packet, b_side.expectedseqnum);
    int iscorrupt = is_corrupted(packet);
    if (iscorrupt){
        printf("B_recv: packet corrupted\n");
    }else if (has_seqnum == 0){
        printf("B_recv: valid packet but seqnum is not expected seqnum\n");
    }else{
        printf("B_recv: valid packet\n");
    }
    // if b receive not-corrupt and correct sequence number message
    // send message to application layer
    if ( iscorrupt == 0 && has_seqnum == 1 ){
        for (int i = 0; i < MSG_SIZE; i++){
            to_application_msg[i] = packet.payload[i];
        }
        p = make_pkt(emptymessage, b_side.expectedseqnum, b_side.expectedseqnum);
        to_application_layer(1, to_application_msg);
        b_side.expectedseqnum += 1;
    // else we don't send message to application layer
    }else{
        p = make_pkt(emptymessage, b_side.expectedseqnum - 1, b_side.expectedseqnum - 1);
    }
    to_network_layer(1, p);  // send the corresponding acknowledgement back to A 
}
...
int nsimmax = 0;   /* number of msgs to generate, then stop */  // initialize in init
float time = 0.000;
float lossprob;    /* probability that a packet is dropped  */   // initialize in init
float corruptprob; /* probability that one bit is packet is flipped */  // initialize in init
float lambda;      /* arrival rate of messages from application layer */ // initialize in init
